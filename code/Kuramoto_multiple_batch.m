...
% @copyright: Quanjun Lang 
%{
 - a typical test;                               
 - batch of independent tests to get stats;    
%}
...
batch_num = 4; %  batch of independent tests to get stats; 
...
plotON    = 1;  % must be 1. a tpyical test, showing trajectory plots; 
...
if plotON ==1       % Plot trajectories and the graph
    ...
    GIF_on = 0;
    if GIF_on==1;  graph_plot_motion_kuramoto(true_path_temp, dyn_sys_temp, 1);  end
end
...
%%
phi_est.in_H.ALS            = cell(Mseq_num, 1);
phi_est.in_H.ORALS          = cell(Mseq_num, 1);
phi_est.in_H.all_rgrid      = cell(Mseq_num, 1);

phi_est.out_H.ALS           = cell(Mseq_num, 1);
phi_est.out_H.ORALS         = cell(Mseq_num, 1);
phi_est.out_H.all_rgrid     = cell(Mseq_num, 1);
...
%%%%%%%%%%%%%%%%%%%%% Kernel est in Hypo %%%%%%%%%%%%%%%%%%%%%%%%%%%%
in_ALS_stats = cell(Mseq_num, 1);
in_ORALS_stats = cell(Mseq_num, 1);
for i = 1:Mseq_num
...
out_ALS_stats = cell(Mseq_num, 1);
out_ORALS_stats = cell(Mseq_num, 1);
for i = 1:Mseq_num
