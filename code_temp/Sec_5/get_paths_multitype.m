function pathObj = get_paths_multitype( dyn_sys, M, varargin )
...
for i = 1:M
    ...
    if ~forcing_noise_provided
        [xpath,forcing_noise{i}]= graph_forward_model_multitype( dyn_sys, X0, progressON);                                         % generate paths, save stoch. forcing term
    else
        xpath                   = graph_forward_model_multitype( dyn_sys, X0, progressON, rk4ON, forcing_noise{i});                       % generate paths, with the stoch. forcing term provided
    end
...
%% save data
if saveON        
    Datadir = graph_forward_model_multitype(M, dyn_sys, learning_setup);    
