function [E, c, E2, c2, all_Z, condA_orals, condL, time] = learn_kernel_graph_ORALS(path_data, dyn_sys, learning_setup, varargin)
% joint learning of kernel and graph from data trajectories
p = inputParser;

% These are all regularization methods available
expected_reg_methods = {'None', 'pinv', 'pinvreg','lsqminnorm', 'ID', 'RKHS', 'RKHS_plain', 'pinv_direct'};
...
if strcmp(reg_method, 'pinv_direct')
    %% This is the case of using pinv directly
    all_A = cell(N, 1);
    all_b = cell(N, 1);
    
    for i = 1:N
        A = zeros(M*d*steps, n*(N-1));
        b = zeros(M*d*steps, 1);
        for m = 1:M
            %             m
            ...
            A((m-1)*d*steps+1:m*d*steps, :) = all_K;
            b((m-1)*d*steps+1:m*d*steps) = dXi_reshape;
        end
        all_A{i} = A;
        all_b{i} = b;
    end
    ...
    ORALS_mat   = kron( learning_setup.dict_mat, graph_mat );                       %MM: if this is ORALS-specific, it should not be computed here, nor be put in I
    ...
    parfor i = 1:N
        ...
        switch reg_method
            ...
            case 'RKHS'
                [~, Z_i] = L_curve(A_i, b_i, 'RKHS', 0, ORALS_mat);
            case 'RKHS_plain'
